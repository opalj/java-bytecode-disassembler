...
export default
{
	...
	disassemble(dir)
	{
		...
		var command =
			'java -jar "' + path.normalize(__dirname + '/OPALDisassembler.jar') + '"' +
			' -source ' + '' + path.normalize(dir) +
			' -o "' + path.normalize(__dirname + '/../temp_views/'+(fileName)+'-disassembled.html') + '"' +
			' -css "' + path.normalize(__dirname + '/../disassembler-styles/disassembledView.css') + '"' +
			' -noDefaultCSS'
		;
		...
		try{
			...
			var newView = new DisassembledView(
				fileName,
				__dirname + '/../temp_views/'+fileName+'-disassembled.html',
				path
			);
			aThis.views[path] = newView;
			return newView;
		}catch(error){
			console.error(error);
